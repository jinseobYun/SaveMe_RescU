...
import com.ssafy.smru.service.DispatchOrderService;
...
public class DispatchOrderController {
    ...
    public ResponseEntity<?> createDispatchOrder(@RequestBody DispatchOrderDto.Request request) {
        try {
            ...
            // 출동지령 정보 저장
            DispatchOrder dispatchOrder = dispatchOrderService.saveDispatchOrder(request, memberId);
            // 저장되면 출동지령 반환
    ...
    public ResponseEntity<?> getDispatchOrder(@PathVariable Long orderId) {
        try {
            DispatchOrderDto.Response response = dispatchOrderService.getDispatchOrder(orderId);
    ...
    public ResponseEntity<?> createDispatchOrder(@RequestBody FirstDispatchOrderDto.FirstInfoRequest dto) {
        try {
            FirstDispatchOrderDto.Response response = dispatchOrderService.createDispatchOrder(dto);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (IllegalArgumentException e) {
